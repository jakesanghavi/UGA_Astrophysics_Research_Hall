...
# --- Habitable zone lines only ---
ax.plot(runaway_greenhouse, mstar, c='blue', lw=3)
ax.plot(maximum_greenhouse, mstar, c='blue', lw=3, label='HZ Boundary')

# --- Horizontal dashed red lines ---
y_levels = [0.1, 0.5, 1.0]
for y in y_levels:
    ax.hlines(
        y=y,
        xmin=0.02,
...
# --- Red scatter points ---
x_points = [
    [0.03321, 0.04538, 0.0575],      # y = 0.1
    [0.2350, 0.3137, 0.3924],        # y = 0.5
    [0.8775, 1.1329, 1.3883]         # y = 1.0
]
y_points = [
    [0.1, 0.1, 0.1],
    [0.5, 0.5, 0.5],
...
hz_points_label = ax.scatter([], [], color='red', s=100,
                             label='10th / 50th / 90th HZ percentiles')

for xs, ys in zip(x_points, y_points):
...
ax.set_title(
    f"Habitable Zone by Stellar Mass and Semimajor Axis of Orbit",
    fontsize=20,
    fontweight='bold'
)
