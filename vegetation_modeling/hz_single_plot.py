...
for idx, MP in enumerate(MPS):
    # Load JSON file
    filename = f"sapelo_{MP}.json"
    if math.floor(MP) == math.ceil(MP):
        filename = f"sapelo_{int(MP)}.json"
    ...
    # Sorted axes for interpolation
    m_keys = sorted([float(k) for k in veg_data.keys()])
    a_keys = sorted([float(k) for k in veg_data[str(m_keys[0])].keys()])

    # Z_data matrix (first element of the veg lists)
    Z_data = np.array([[veg_data[str(m)][str(a)][0] for a in a_keys] 
                       for m in m_keys])
    ...
    # ----- Evaluate on your full grid (these MUST be pre-defined) -----
    # X = mesh of AU values, Y = mesh of stellar masses
    ...
    HZ_mask = (X >= runaway_interp[:, None]) & (X <= maximum_interp[:, None])
    Z_masked = np.ma.masked_where(~HZ_mask, Z)
    

    # ----- Plotting -----
    cmap = mcolors.LinearSegmentedColormap.from_list(
        "veg_map", ["red", "yellow", "green"]
    )
    norm = mcolors.LogNorm(vmin=1e-10, vmax=1e-9)

    pcm = ax.pcolormesh(X, Y, Z_masked, cmap=cmap, norm=norm, shading='auto')
    ax.plot(runaway_greenhouse, mstar, c='blue', lw=3)
    ax.plot(maximum_greenhouse, mstar, c='blue', lw=3)
    ...
    ax.set_title(
        rf"GPP Map for FI={F_INIT}, MP={MP} M$_\oplus$",
        fontsize=14, fontweight="bold"
    )
    ax.tick_params(direction='in', length=12, width=2)
    
target_masses = [0.1, 0.5, 1.0]
percentiles = [10, 50, 90]

results = {}

for m_target in target_masses:
    # Find closest row in Y grid
    idx = np.argmin(np.abs(Y[:, 0] - m_target))

    x_row = X[idx, :]
    mask_row = HZ_mask[idx, :]

    # Keep only HZ values
    x_valid = x_row[mask_row]

    if len(x_valid) == 0:
        results[m_target] = [np.nan]*3
        continue

    # Compute percentiles (UNWEIGHTED)
    p_values = np.percentile(x_valid, percentiles)

    results[m_target] = p_values
    
for m, vals in results.items():
    print(f"M = {m} Msun → X percentiles (AU):")
    print(f"  10%: {vals[0]:.3f}, 50%: {vals[1]:.3f}, 90%: {vals[2]:.3f}")
...
cbar.set_label(r'GPP [kg C $\mathbf{m^{-2}s^{-1}}$]', fontsize=12)

# Shared axis labels
fig.supxlabel('Planet semi-major axis [au]', fontsize=16, fontweight='bold')
fig.supylabel(r'Host star mass [M$_\odot$]', fontsize=16, fontweight='bold')
